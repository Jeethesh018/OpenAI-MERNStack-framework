    ...
    <div className={`flrx flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b
     ...
     backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && "max-md:-translate-x-full"}`}>
      <img className='w-full max-w-48'
          ...
          chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=> (
            ...
            rounded-md cursor-pointer flex justify-between group'>
               <div>
                ...
                <p className='text-sx text-gray-500 dark:text-[#B1A6CO]]'>{moment(chat.updatedAt).fromNow()}</p>
                </div>
       ...
       <div 
        className='flex items-center gap-2 p-3 mt-4 border border-gray-300
       dark:border-white/15 rounded-md justify-between'>
         ...
         <label className='relative inline-flex cursor-pointer'>
            ...
            <div className='w-9 h-5 bg-gray-400 rounded-full peer-checked:bg-purpose-600 transition-all'>
...
<div
       ...
       <img src={assets.close_icon} onClick={()=>setIsMenuOpen(false)} className='absolute top-3 right-3 w-5 h-5
